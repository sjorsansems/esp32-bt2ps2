...
// Set to printf(__VA_ARGS__) when you want verbose BT/BLE scan output.
...
std::map<std::pair<esp_hidh_dev_t *, uint16_t>, BTKeyboard::hid_report_keyboard> BTKeyboard::keyboard_reports;
...
void BTKeyboard::hidh_callback(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
  ...
  case ESP_HIDH_INPUT_EVENT:
  {
    ...
    {
      key_pair = std::make_pair(param->input.dev, param->input.report_id); // construct the key for the map (multi-device support)
      bt_keyboard->push_key(param->input.data, param->input.length, &key_pair);
    }
...
{
  Mouse_Control mouse;
  static bool mouse_log_initialized = false;
  ...
  if (!mouse_log_initialized)
  {
    ...
    mouse_log_initialized = true;
  }

  if (mouse.mouse_buttons != prev_buttons)
  {
...
void BTKeyboard::push_key(uint8_t *keys, uint8_t size, std::pair<esp_hidh_dev_t *, uint16_t> *key_pair)
{
  KeyInfo inf = {};
  inf.modifier = (size > 0) ? (KeyModifier)keys[0] : (KeyModifier)0;

  if ((key_pair != nullptr) && (keyboard_reports.count(*key_pair) > 0) && !keyboard_reports[*key_pair].array_usages.empty())
  {
    const hid_report_keyboard &report = keyboard_reports[*key_pair];
    uint8_t out = 0;
    uint16_t bit_count = size * 8;
    uint16_t max_bits = (uint16_t)report.array_usages.size();
    ...
    for (uint16_t bit = 0; (bit < bit_count) && (out < MAX_KEY_COUNT); bit++)
    {
      uint8_t byte = keys[bit >> 3];
      uint8_t mask = (uint8_t)(1U << (bit & 0x07));

      if (byte & mask)
      {
        uint16_t usage = report.array_usages[bit];

        if (usage >= 0xE0 && usage <= 0xE7)
        {
          uint8_t mod_bit = (uint8_t)(1U << (usage - 0xE0));
          inf.modifier = (KeyModifier)(((uint8_t)inf.modifier) | mod_bit);
        }
        else if (usage >= 0x04 && usage <= 0xE7)
        {
    ...
    if (out < MAX_KEY_COUNT)
    {
      inf.keys[out] = 0;
    }

    ESP_LOGD(TAG, "Keyboard report parsed using descriptor map: size=%u keys=%u", size, out);
    ESP_LOG_BUFFER_HEX_LEVEL("KEYS PASSED TO MAIN: ", inf.keys, out, ESP_LOG_DEBUG);
  }

  // Fallback path: if descriptor parsing produced no usable output while report bytes are active,
  // decode using legacy offsets so typing never disappears.
  bool has_raw_activity = false;
  for (int i = 0; i < size; i++)
  {
    if (keys[i] != 0)
    {
      has_raw_activity = true;
      break;
    }
  }

  bool has_decoded_activity = (((uint8_t)inf.modifier) != 0);
  if (!has_decoded_activity)
  {
    for (int i = 0; i < MAX_KEY_COUNT; i++)
    {
      if (inf.keys[i] != 0)
      {
        has_decoded_activity = true;
        break;
      }
    }
  }

  // 8BitDo-style 16-byte keyboard reports: scan bytes 1-15 as NKRO bitmap.
  // For this device, usage numbering starts at 0x00 in the bitmap payload.
  if (size >= 16 && has_raw_activity && !has_decoded_activity)
  {
    uint8_t out = 0;
    inf.modifier = (size > 0) ? (KeyModifier)keys[0] : (KeyModifier)0;
    ...
    has_decoded_activity = (out > 0) || (((uint8_t)inf.modifier) != 0);

    if (has_decoded_activity)
    {
      ESP_LOGD(TAG, "Keyboard report parsed as NKRO: size=%u keys=%u", size, out);
    }
  }
...
int BTKeyboard::handle_report(hid_report_params_t *report, esp_hidh_dev_t *device)
{
  ...
  {
    keyboard_reports[key].report_id = report->report_id;
    keyboard_reports[key].input_len = report->input_len;
    keyboard_reports[key].report_count = BTKeyboard::s_report_count;
    keyboard_reports[key].contains_array = report->contains_array;
    keyboard_reports[key].array_usages = temp_usages_array;

    ESP_LOGD("bt_Keyboard handle_report: ", "Amount of KEYBOARD reports: %u entries=%u", keyboard_reports.size(), keyboard_reports[key].array_usages.size());
  }
