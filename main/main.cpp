...
const bool serial_show_raw_hid = false; // set true for raw [KEYBOARD]/[MOUSE] dumps
...
static char hid_to_ascii(uint8_t hid, bool shift)
{
    ...
    if (hid >= 0x1e && hid <= 0x27)
    {
        ...
        uint8_t idx = hid - 0x1e;
        return shift ? nums_shift[idx] : nums[idx];
    }
    if (hid == 0x2c) return ' ';
    if (hid == 0x2d) return shift ? '_' : '-';
    if (hid == 0x2e) return shift ? '+' : '=';
    if (hid == 0x2f) return shift ? '{' : '[';
    if (hid == 0x30) return shift ? '}' : ']';
    if (hid == 0x31) return shift ? '|' : '\\';
    if (hid == 0x33) return shift ? ':' : ';';
    if (hid == 0x34) return shift ? '"' : '\'';
    if (hid == 0x35) return shift ? '~' : '`';
    if (hid == 0x36) return shift ? '<' : ',';
    if (hid == 0x37) return shift ? '>' : '.';
    if (hid == 0x38) return shift ? '?' : '/';

    return 0;
}

static const char *hid_special_name(uint8_t hid)
{
    switch (hid)
    {
...
extern "C"
{
    ...
    void app_main(void)
    {
        ...
        if (bt_keyboard.setup(pairing_handler)) // Must be called once
        {
            ...
            {
                // Delay startup pairing to let auto-reconnect settle first.
                // This prevents pairing scan from racing against bonded reconnect.
                gpio_set_level(GPIO_NUM_2, 1);
            }
            else
            {
        ...
        bool startup_pairing_pending = pairing_at_startup;
        TickType_t startup_pairing_deadline = xTaskGetTickCount() + pdMS_TO_TICKS(15000);
        ...
        while (true)
        {
            if (startup_pairing_pending)
            {
                if (BTKeyboard::isConnected)
                {
                    startup_pairing_pending = false;
                    ESP_LOGI(TAG, "Startup reconnect successful. Startup pairing skipped.");
                }
                else if (xTaskGetTickCount() >= startup_pairing_deadline)
                {
                    int bonded = esp_ble_get_bond_device_num();
                    if ((bonded == 0) && !pairingRequested)
                    {
                        ESP_LOGI(TAG, "No bonded devices after startup grace period. Starting pairing scan.");
                        pairingRequested = true;
                        start_pairing_scan();
                    }
                    else
                    {
                        ESP_LOGI(TAG, "Bonded devices present (%d). Startup pairing not needed.", bonded);
                    }
                    startup_pairing_pending = false;
                }
            }
            ...
            {
                if (serial_show_raw_hid)
                {
                    // Print raw keycodes to serial terminal
                    printf("[KEYBOARD] Modifier: 0x%02X | Keys:", (uint8_t)info.modifier);
                    for (int i = 0; i < BTKeyboard::MAX_KEY_COUNT; i++)
                    {
                        printf(" 0x%02X", info.keys[i]);
                    }
                    printf("\n");
                }
                ...
                for (int i = 0; i < BTKeyboard::MAX_KEY_COUNT; i++)
                {
                    ...
                    if (!found)
                    {
                        ...
                        bool shift_active = (((uint8_t)info.modifier & 0x02) != 0) || (((uint8_t)info.modifier & 0x20) != 0);
                        char typed = hid_to_ascii(info.keys[i], shift_active);
                        if (typed)
                        {
                            printf("%c", typed);
                            fflush(stdout);
                        }
                        else if (info.keys[i] == 0x28) // Enter
                        {
                            printf("\n");
                            fflush(stdout);
                        }
                        ...
                        {
                            const char *special = hid_special_name(info.keys[i]);
                            if (special)
                            {
                                printf("[%s]", special);
                                fflush(stdout);
...
void mouse_task(void *arg)
{
    ...
    while (true)
    {
        if (bt_keyboard.wait_for_low_event_MOUSE(infoMouse)) // return immediately if queue empty
        {
            if (serial_show_raw_hid)
            {
                printf("[MOUSE] Buttons: 0x%02X | X: %d | Y: %d | W: %d\n", infoMouse.mouse_buttons, infoMouse.mouse_x, infoMouse.mouse_y, infoMouse.mouse_w);
            }
